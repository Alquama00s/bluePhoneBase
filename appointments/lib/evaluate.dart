...
class Evaluate extends StatefulWidget {
  ...
  final Map<String,dynamic> rateList,view;
  final double base;
  Evaluate(this.quest, this.option, this.rateList, this.base,this.view);
  @override
...
class Entry extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: getFire(view['phonestate']['root']),
      builder: (context,snap){
        if(snap.hasError){
          return Error('Something went wrong');
        }
        if(snap.connectionState==ConnectionState.done){
          return Evaluate(quest, option, snap.data['ratelist'], snap.data['price'],view);
        }
        return Loading();
      },
    );
            ...
            RaisedButton(
              ...
              onPressed: () => {
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => Send(reSend(rate,root,hashid),Loader()))),
              },
              color: Colors.green[600],
              child: Text(
                'Send Now',
